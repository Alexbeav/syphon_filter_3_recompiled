...
def main() -> int:
    ...
    openbios_image = ROOT / "bios" / "openbios.bin"
    if openbios_image.is_file():
        shutil.copy2(openbios_image, framework / "bios")
    else:
        print("PSXRecomp CLI package: OpenBIOS image absent; source-only "
              "package will require a locally built or user-supplied BIOS")

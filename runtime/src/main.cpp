...
static bool keyboard_input_focused(void) {
    return !g_headless && !g_hidden_window && sdl_window &&
...
static bool hybrid_dpad_active(const PlayerInput& p, int player, bool dev_any) {
    ...
    if ((p.kind == 1 || dev_any) && keyboard_input_focused()) {
        const Uint8* keys = SDL_GetKeyboardState(NULL);
...
session_reboot:
    ...
    std::fprintf(stdout,
                 "psxrecomp: geometry precision %s; perspective textures %s; precise culling %s\n",

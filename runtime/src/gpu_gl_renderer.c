...
static void glb_fill_rect(int x,int y,int w,int h,uint16_t c){
     ...
     * switch must update that representation as well as the FBO; otherwise
     * uncovered movie rows retain arbitrary pre-movie CPU data. */
...
static void depth24_set_mode(int d24) {
    ...
    if (d24 && !s_depth24_skip_up) {
        /* Finish all 15-bit FBO work before packed RGB888 uploads make CPU
         * VRAM authoritative. This must precede the first CPU upload or the
         * readback would overwrite part of that movie. */
        ensure_cpu();
        s_up_nrects = 0;
        rect_clear(&s_d24_skip_fb);
